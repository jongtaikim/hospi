...
class mdl_online extends CI_Model {
	...
	function load_view($idx=''){
		...
		if($idx){
			...
			$sql = "select a.email, a.phone,a.subject, a.title, a.name, a.id, a.date, a.ind, a.ref, b.text, a.comment, a.pw , a.product_name
	...
	function online_add($data){
        ...
        $post['mail_to'] = $data['email'];
        $post['mail_from'] = "hospi@hospi.co.kr";
        $post['name'] = $data['name'];
        $post['title'] = "[".$data['subjectext']."] ".$data['name']."님의 문의가 등록되었습니다. ";
        $post['msg'] = "이름 : ".$data['name']."<br><br>";
        $post['msg'] = "날짜 : ".date("Y-m-d")."<br><br>";
        $post['msg'] .= "전화 : ".$data['phone']."<br><br>";
        $post['msg'] .= "제품명 : ".$data['product_name']."<br><br>";
        $post['msg'] .= "이메일 : ".$data['email']."<br><br>";

        $post['msg'] .= "내용 : ".nl2br($data['comment'])."<br><br>";

 
		$this->load->library('email');
		$this->email->from($post['mail_from']);
		//$this->email->to($post['mail_to'],$post['name']); 

		$this->email->cc("hospicare83@naver.com"); 
		$this->email->bcc("jongtainow17@gmail.com"); 
		$this->email->to("hospi@hospi.co.kr"); 
