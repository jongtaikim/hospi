<?php /* Template_ 2.2.3 2016/07/28 14:34:17 /home/hosting_users/hospi/www/application/util/openeditor/editor.html */
$TPL_img_file_LIST_1=empty($TPL_VAR["img_file_LIST"])||!is_array($TPL_VAR["img_file_LIST"])?0:count($TPL_VAR["img_file_LIST"]);?>
<link rel="stylesheet" href="/application/util/openeditor/css/editor.css" type="text/css"  charset="utf-8"/>
<!-- modify document.domain with yours if you want XSS -->
<!--<script type="text/javascript">document.domain = 'daum.net';</script>-->
<script type="text/javascript" charset="utf-8" src="/application/util/openeditor/js/editor_loader.js"></script>
<script type="text/javascript" charset="utf-8" src="/trunk/scripts/lib.validate2.js"></script>
<div class="body">
<!-- 에디터 시작 -->
<!--
@decsription
...
<li class="tx-list">
<div unselectable="on" class="	tx-btn-lbg	  	tx-table" id="tx_table">
...
<li class="tx-list">
<div unselectable="on" class="tx-btn-bg tx-mergecells" id="tx_mergecells">
...
<div id="tx_loading" class="tx-loading"><div><img src="/application/util/openeditor/images/icon/editor/loading2.png" width="113" height="21" align="absmiddle"/></div></div>
...
<img id="tx_resize_holder" src="/application/util/openeditor/images/icon/editor/skin/01/btn_drag01.gif" width="58" height="12" unselectable="on" alt="" />
</div>
...
<div id="tx_attach_preview" class="tx-attach-preview" style="padding:0px"><p></p><img src="/application/util/openeditor/images/icon/editor/pn_preview.gif" width="147" height="108" unselectable="on"/></div>
<div class="tx-attach-main">
<div id="tx_upload_progress" class="tx-upload-progress"><div>0%</div><p>파일을 업로드하는 중입니다.</p></div>
<ul class="tx-attach-top">
<li id="tx_attach_delete" class="tx-attach-delete" style="display:none"><a>전체삭제</a></li>
...
form: '<?php echo $TPL_VAR["form"]?>'+"", /* 등록하기 위한 Form 이름 */
txIconPath: "/application/util/openeditor/images/icon/editor/", /*에디터에 사용되는 이미지 디렉터리, 필요에 따라 수정한다. */
txDecoPath: "/application/util/openeditor/images/deco/contents/", /*본문에 사용되는 이미지 디렉터리, 서비스에서 사용할 때는 완성된 컨텐츠로 배포되기 위해 절대경로로 수정한다. */
canvas: {
styles: {
},
showGuideArea: true
...
size: {
contentWidth: '<?php echo $TPL_VAR["width"]?>' /* 지정된 본문영역의 넓이가 있을 경우에 설정 */
}
};
EditorJSLoader.ready(function(Editor) {
Trex.Canvas.TextAreaProcessor.prototype.focusOnTop = function () {
//do nothing.
};
...
if(validate(document.getElementById('<?php echo $TPL_VAR["form"]?>'))){
reload_filelist();
...
* @function
* @param <?php echo $TPL_VAR["Object"]?> editor - 에디터에서 넘겨주는 editor 객체
* @returns <?php echo $TPL_VAR["Boolean"]?> 모든 데이터가 유효할 경우에 true
*/
...
* @function
* @param <?php echo $TPL_VAR["Object"]?> editor - 에디터에서 넘겨주는 editor 객체
* @returns <?php echo $TPL_VAR["Boolean"]?> 정상적인 경우에 true
*/
...
tx.textarea({
'name': "<?php echo $TPL_VAR["name"]?>", // 본문 내용을 필드를 생성하여 값을 할당하는 부분
'style': { 'display': "none" }
}, content)
);
$('#<?php echo $TPL_VAR["name"]?>').val(content);
$('textarea[name=<?php echo $TPL_VAR["name"]?>]').val(content);
//alert($('#<?php echo $TPL_VAR["name"]?>').val());
...
tx.input({
'type': "hidden",
'name': 'attach_img_file['+i+'][files]',
'value': images[i].data.originalurl // 예에서는 이미지경로만 받아서 사용
})
);
formGenerator.createField(
tx.input({
'type': "hidden",
'name': 'attach_img_file['+i+'][filename]',
'value': images[i].data.filename // 예에서는 이미지경로만 받아서 사용
})
);
formGenerator.createField(
tx.input({
'type': "hidden",
'name': 'attach_img_file['+i+'][filesize]',
'value': images[i].data.filesize // 예에서는 이미지경로만 받아서 사용
})
);
formGenerator.createField(
tx.input({
'type': "hidden",
'name': 'attach_img_file['+i+'][thumburl]',
'value': images[i].data.thumburl // 예에서는 이미지경로만 받아서 사용
})
);
formGenerator.createField(
tx.input({
'type': "hidden",
'name': 'attach_img_file['+i+'][filemime]',
'value': images[i].data.filemime // 예에서는 이미지경로만 받아서 사용
})
);
}
}
...
tx.input({
'type': "hidden",
'name': 'attach_file['+i+'][files]',
'value': files[i].data.originalurl // 예에서는 이미지경로만 받아서 사용
})
);
formGenerator.createField(
tx.input({
'type': "hidden",
'name': 'attach_file['+i+'][filename]',
'value': files[i].data.filename // 예에서는 이미지경로만 받아서 사용
})
);
formGenerator.createField(
tx.input({
'type': "hidden",
'name': 'attach_file['+i+'][filesize]',
'value': files[i].data.filesize // 예에서는 이미지경로만 받아서 사용
})
);
formGenerator.createField(
tx.input({
'type': "hidden",
'name': 'attach_file['+i+'][filemime]',
'value': files[i].data.filemime // 예에서는 이미지경로만 받아서 사용
})
);
}
return true;
...
<textarea id="<?php echo $TPL_VAR["name"]?>" style="display:none;"><?php echo $TPL_VAR["content"]?></textarea>
<input type="hidden" name="sect" id="sect" value="<?php echo $TPL_VAR["sect"]?>">
<script type="text/javascript">
function loadContent() {
var attachments = {};
attachments['image'] = [];
<?php if($TPL_img_file_LIST_1){foreach($TPL_VAR["img_file_LIST"] as $TPL_V1){?>
<?php if($TPL_V1["str_type"]=="image"){?>
attachments['image'].push({
'attacher': 'image',
'data': {
'imageurl': 'http://<?=$_SERVER[HTTP_HOST]?><?php echo $TPL_V1["pe_url"]?>',
'filename': '<?php echo $TPL_V1["str_upfile"]?>',
'filesize': <?php echo $TPL_V1["num_size"]?>,
'originalurl': '<?php echo $TPL_V1["str_refile"]?>',
'thumburl': '<?php echo $TPL_V1["str_thum"]?>',
'imagealign': 'C'
}
});
<?php }?>
...
'attacher': 'file',
'data': {
'attachurl': 'http://<?=$_SERVER[HTTP_HOST]?><?php echo $TPL_V1["pe_url"]?>',
'filemime': '<?php echo $TPL_V1["str_mime"]?>',
'originalurl': '<?php echo $TPL_V1["str_refile"]?>',
'filename': '<?php echo $TPL_V1["str_upfile"]?>',
'filesize': <?php echo $TPL_V1["num_size"]?>
}
...
"content": document.getElementById("<?php echo $TPL_VAR["name"]?>") /* 내용 문자열, 주어진 필드(textarea) 엘리먼트 */
});
}
loadContent();
...
reload_filelist();
setTimeout(function(){
$('.tx-name').each(function(){
var t_name = $(this).html();
$(this).next().find('.tx-delete').each(function(){
$(this).attr('filename',t_name).attr('str_sect','<?php echo $TPL_VAR["sect"]?>');
});
});
},1000);
});
function del_filelist(fname,sect){
$.ajax({
type: 'GET',
url: '/plug_daum_editor_upload/delete_file/?filename='+fname+'&str_sect='+sect,
dataType: 'html',
...
$.ajax({
type: 'GET',
url: '/plug_daum_editor_upload/list_files/?str_sect=<?php echo $TPL_VAR["sect"]?>',
dataType: 'html',
success: function(html, status) {
$('#file_lists').html(html);
},
error: function(request,status,error) {
alert(request.responseText);
}
